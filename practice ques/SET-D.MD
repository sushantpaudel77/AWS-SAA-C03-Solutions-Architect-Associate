...
|----------|---------|--------------|
| 196 | D | DynamoDB TTL, Cost optimization |
| 197 | B, E | Elastic Beanstalk, RDS Multi-AZ, Minimal changes |
| 198 | D | EKS, DocumentDB, No code changes |
| 199 | B | Amazon Transcribe, Amazon Athena |
| 200 | D | Cognito User Pool Authorizer, Least overhead |
| 201 | B | Amazon Pinpoint, Kinesis Data Streams |
| 202 | B | KMS Customer Managed Key, Auto rotation |
| 203 | D | Auto Scaling, SQS queue depth metrics |
| 204 | C | AWS Lake Formation, Fine-grained permissions |
| 205 | C | S3 static hosting, CloudFront OAI |
| 206 | C | EventBridge, API call monitoring |
| 207 | D | SQS buffer, Write throttling solution |
| 208 | A | VPC Interface Endpoint for S3, IAM policies |
| 209 | A | ElastiCache, Distributed sessions |
| 210 | D | SQS queues, Backlog per instance metrics |
| 211 | D | Resource Groups Tag Editor, Global search |
| 212 | A | S3 Intelligent-Tiering, Variable access |
| 213 | A | AWS WAF, Application-level protection |
| 214 | B | AWS Glue ETL, Least development effort |
| 215 | A | AWS Snowball, S3 Glacier Deep Archive |
| 216 | B | S3 default encryption, S3 Batch Operations |
| 217 | A | Active-passive DR, Aurora cross-region replica |
| 218 | A, E | Security groups, Network ACLs, Stateful vs stateless |
| 219 | D | R5 instances, CloudWatch agent, Memory optimization |
| 220 | B | AWS Lambda, Variable workload, Cost optimization |
| 221 | D | Amazon S3, Long-term storage, Concurrent access |
| 222 | A | Cross-account IAM roles, Least privilege |
| 223 | A, D | EKS service accounts, VPC endpoints |
| 224 | C, E | Route 53 multivalue, Multi-AZ deployment |
| 225 | B | Kinesis Data Firehose, Redshift, SQL analytics |
| 226 | A, E | API Gateway + Kinesis, AWS Glue processing |
| 227 | B | S3 Lifecycle, Version management |
| 228 | C | SQS buffering, Lambda processing |
| 229 | A | Aurora Serverless, Auto scaling |
| 230 | C | NAT Gateways, Multi-AZ, High availability |
| 231 | B | VPC Peering, Secure connectivity |
...
**Cost Optimization**
- Use DynamoDB TTL for automatic data expiration (Q196)
- Choose appropriate S3 storage classes based on access patterns (Q212)
- Leverage serverless services for variable workloads (Q220)

**High Availability & Fault Tolerance**
- Deploy across multiple Availability Zones (Q224, Q230)
- Use managed services to reduce operational overhead (Q229)
- Implement proper disaster recovery strategies (Q217)

**Security Best Practices**
- Use IAM roles instead of embedded credentials (Q222, Q223)
- Implement least privilege access (Q208, Q231)
- Use VPC endpoints for private connectivity (Q208, Q223)

**Scalability Patterns**
- Use queues to decouple and buffer traffic (Q207, Q210, Q228)
- Implement proper Auto Scaling metrics (Q203, Q210)
- Choose appropriate compute services for workload patterns (Q220)

**Data Management**
- Understand different database migration strategies (Q197)
- Use appropriate data processing services (Q214, Q225)
- Implement proper data lifecycle management (Q215, Q227)

## Study Tips

1. **Understand AWS Service Limits**: Know when to use buffering patterns like SQS
2. **Cost vs Performance Trade-offs**: Understand when to choose different storage classes and compute options
3. **Security Layers**: Remember that AWS uses defense in depth - security groups, NACLs, IAM, etc.
4. **Managed vs Self-Managed**: AWS generally prefers managed services for reduced operational overhead
5. **Multi-AZ vs Multi-Region**: Understand the difference and when to use each
...
- Suggesting additional study materials
